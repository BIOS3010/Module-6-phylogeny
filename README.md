...
Myg_Phyca = myoglobin from Physeter catodon = sperm whale)
...
You can set up the the pipline after choosing the “A la Carte” option (see figure):
...
-	If you click “step by step” you will get an output for each step of the pipeline. This allows you to inspect the result for each step of the pipeline as well as make adjustments (if necessary) and save intermediate files for later use.

Now click “create workflow”. This will take you to a new page where you can paste the hemoglobin sequences from the previous page (or use the file Ex1_globin.fasta)  Hit “submit” and the sequence alignment will be constructed. The graphical output shows you how conserved the characters in the alignment are compare to the BLOSUM62 score:  Max: 3.0      Mid: 1.5      Low: 0.5 . Higher scoring regions are more conserved. You are given the option to save the MSA in several formats if you want to. This is not strictly necessary for this excerisce since the alignment was done quite quickly. But if it had taken a long time (for larger alignments this step can take hours!) you probably would have wanted to save the result for safekeeping.
...
Calculate a tree using Muscle and ML with Approximate Likelihood-Ratio Test (aLRT) with  these sequences (Ex3_alphacrystaline_v2.fasta).  :
