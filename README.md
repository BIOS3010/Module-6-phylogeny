...
You can set up the pipeline after choosing the "A la Carte" option. When you set up the pipeline choose **Step by step**  which will provide an output for each step of the pipeline and allows you to inspect the result and set up extra parameters for each step.
...
Each groups should provide the manullay drawn tree, a screenshot (or "proper" figure file) of the tree they generated with the pipeline. And answer these questions:
...
Use these protein sequences in the file [Ex2_alphacrystalline.fasta](Ex2_alphacrystalline.fasta) :


Calculate the tree with _Muscle_, _"less stringent"_ curation setting and _ML_, this time choose "Approximate Likelihood-Ratio Test (aLRT)" instead of bootstrap since it is much faster.
```diff
Look at the tree in FigTree. Any surprises?
```
## Exercise 6.3
**A tree with an "error"?**
Calculate a tree using Muscle and ML with Approximate Likelihood-Ratio Test (aLRT) with sequences [Ex3_alphacrystaline_v2.fasta](Ex3_alphacrystaline_v2.fasta).  :
